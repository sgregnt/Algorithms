...
namespace TestFramework
{
   ...
   public:
      ...
      void Split(char delimiter, StringSegment& prefix);
   ...
   bool Parse(StringSegment ssegement, std::vector<int>& result)
   {
      ...
      StringSegment suffix(ssegement);
   ...
   {
      AddColumn<T, int>(prAdapter, "problem", &T::id);
   ...
   void StringSegment::Split(char delimiter, StringSegment& prefix)
   {
      assert(this != &prefix);
   ...
   void WriteRecordToStream(std::ostream& out, ITable* table, size_t row, bool bWriteDefaultValues, bool bIndent)
   {
   ...
   void WriteTableToStream(std::ostream& out, ITable* table, ITable* header = nullptr, bool bWriteDefaultValues = false)
   {
   ...
   void WriteTableToFile(const char* filename, ITable* header, ITable* table = nullptr, bool bWriteDefaultValues = false)
   {
      std::ofstream out;
      out.open(filename);
      ExitIfConditionFails(out.good(), "Cannot open output file!");

      WriteTableToStream(out, header, table);
   ...
   void ProcessResults(std::vector<T>& problems, ProblemSetHeader& header)
   {
      ...
      if (nMistakes > 0)
      {
         std::cout << std::endl << "Your algorithm made " << nMistakes << " mistake(s)." << std::endl << std::endl;
      }
      else
      {
         std::cout << "Your algorithm solved all test problems correctly. Congratulations!" << std::endl << std::endl;
      }
   }
}
